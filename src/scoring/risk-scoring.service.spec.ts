...
import { ViabilityRiskBreakdown } from './interfaces/pole-viability-score.interface';
...
describe('RiskScoringService', () => {
  ...
  it('returns dataAvailable=false with overallRisk=null when soil data is unavailable, and logs why', () => {
  ...
  it('returns dataAvailable=false when current conditions are unavailable', () => {
    const score = service.calculateViabilityScore(
      reginaSoil(),
      reginaClimate(),
      { ...reginaCurrentConditions(), dataAvailable: false, conditions: null },
    );

    expect(score.dataAvailable).toBe(false);
  });

  it('returns dataAvailable/overallRisk/shortTermRisk/longTermRisk in the public result, not contributingFactors', () => {
    const score = service.calculateViabilityScore(
      reginaSoil(),
      reginaClimate(),
      reginaCurrentConditions(),
  ...
  it('rounds overallRisk to 2 decimal places for display, while the logged value retains full precision', () => {
    ...
    expect(score.overallRisk).toBe(Math.round(score.overallRisk! * 100) / 100);
  ...
  it('computes a plausible low-moderate risk for Regina on a calm, well-drained summer day', () => {
    ...
    expect(breakdown.contributingFactors.longTerm.footingDepthRisk).toBeCloseTo(
      0.286,
      2,
    );
  });
  ...
  it('every risk field stays within [0, 1] regardless of how extreme the inputs are', () => {
    const score = service.calculateViabilityScore(
      reginaSoil({
        ...
        landform: { slopePercent: 60, slopeSegmentPercent: 100, name: 'Steep' },
      }),
      reginaClimate({
        normals: {
          meanWindSpeedKmh: 60,
  ...
  it('gives loose soil (low bulk density) a strictly higher footing-depth risk than dense soil, holding depth class constant', () => {
    ...
    expect(loose.contributingFactors.longTerm.bulkDensityRisk).toBeGreaterThan(
      dense.contributingFactors.longTerm.bulkDensityRisk,
    );
    expect(dense.contributingFactors.longTerm.bulkDensityRisk).toBeCloseTo(
      0,
      5,
    );
    expect(loose.contributingFactors.longTerm.footingDepthRisk).toBeGreaterThan(
      dense.contributingFactors.longTerm.footingDepthRisk,
    );
  });

  it('gives a wind gust far above normal a strictly higher short-term risk than calm conditions', () => {
    const calm = service.calculateViabilityScore(
      reginaSoil(),
      reginaClimate(),
      reginaCurrentConditions(),
    );
    const gusty = service.calculateViabilityScore(
      reginaSoil(),
      reginaClimate(),
      reginaCurrentConditions({
        conditions: {
  ...
  it('peaks freeze-thaw transition risk when current temperature is exactly 0°C on wet soil', () => {
    ...
    expect(
      ...
      wellAboveFreezing.contributingFactors.shortTerm.freezeThawTransitionRisk,
    );
  });
